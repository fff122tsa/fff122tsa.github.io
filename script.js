...
let wall = generateFood();
let direction = "right";
let isGameStarted = false;
let gameSpeedDeley = 250;
let highScore = 0;
let gameIntervalId;
...
function checkCollision() {
    ...
    if (head.x < 1 || head.x > gridSize ||
        ...
        head.x< 1 || head.x > Stenka  ||
        head.y< 1 || head.y > Stenka ){
        resetGame();
...
function Stenka() {
    let curentScore = snake.length - 1;
    if (curentScore >= 4) {
        let StenkaElement = createElement("div", "Stenka");
        setPosition(StenkaElement,wall);
        board.appendChild(StenkaElement);
        clearInterval(gameIntervalId);
        gameIntervalId = setInterval(() => {
            move();
            checkCollision();
